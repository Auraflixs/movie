...
window.addEventListener('load', () => {
    injectCustomPlayerStyles(); // Inyectamos los estilos del reproductor
...
// ==========================================
// ESTILOS DEL REPRODUCTOR PERSONALIZADO (CSS-IN-JS)
// ==========================================
function injectCustomPlayerStyles() {
    const styleId = 'auraflix-player-styles';
    ...
    const css = `
        ...
        /* Video Element */
        .aura-player-wrapper video {
            width: 100%;
            height: 100%;
            object-fit: contain;
        }

        /* Overlay de Controles (Gradiente) */
        .aura-controls {
            position: absolute;
            bottom: 0;
            left: 0;
            ...
            pointer-events: none; /* Para que no bloquee clics si está oculto */
        }

        /* Mostrar controles al hacer hover o pausa */
        .aura-player-wrapper:hover .aura-controls,
        .aura-player-wrapper.paused .aura-controls {
            opacity: 1;
            pointer-events: all;
        }

        /* Botón Play Central Grande */
        .aura-big-play {
            position: absolute;
            top: 50%;
            left: 50%;
            ...
            background: rgba(138, 43, 226, 0.4); /* Morado transparente */
            width: 80px;
            height: 80px;
            border-radius: 50%;
            display: flex;
        ...
        .aura-player-wrapper.paused .aura-big-play {
            opacity: 1;
        ...
        .aura-progress-container:hover { height: 8px; }

        .aura-progress-bar {
            height: 100%;
            background: #8A2BE2; /* Morado Auraflix */
            width: 0%;
            border-radius: 5px;
            position: relative;
        }
        ...
        .aura-progress-container:hover .aura-progress-thumb { opacity: 1; }
        ...
        .aura-btn:hover { opacity: 1; color: #8A2BE2; }

        .aura-time {
            font-size: 12px;
            font-family: monospace;
            color: #ddd;
            margin-left: auto; /* Empuja el tiempo a la derecha si es necesario */
            margin-right: 15px;
        }

        /* En Móvil */
        @media (max-width: 768px) {
            .aura-controls { padding: 10px; }
            .aura-btn { font-size: 22px; margin-right: 20px; }
        }
    `;
...
// ==========================================
// FUNCIÓN PLAYER VIDEO ACTUALIZADA CON CUSTOM PLAYER
// ==========================================
function setPlayerVideo(url, overlayText = null) {
    ...
    if (isDirectFile) {
        // --- USAR CUSTOM PLAYER PARA ARCHIVOS DIRECTOS ---
        createCustomPlayer(url, playerDiv);
    } else {
        // --- USAR IFRAME PARA YOUTUBE/VIMEO/EMBEDS ---
...
// --- LÓGICA DEL CUSTOM PLAYER ---
function createCustomPlayer(url, container) {
    const wrapper = document.createElement('div');
    wrapper.className = 'aura-player-wrapper paused';
    
    wrapper.innerHTML = `
        <video src="${url}" playsinline preload="metadata"></video>
        <div class="aura-big-play"><i class="fas fa-play"></i></div>
        <div class="aura-controls">
            <div class="aura-progress-container">
                <div class="aura-progress-bar"><div class="aura-progress-thumb"></div></div>
            </div>
            <div class="aura-controls-row">
                <div style="display:flex;align-items:center;">
                    <button class="aura-btn btn-play-pause"><i class="fas fa-play"></i></button>
                    <span class="aura-time">00:00 / 00:00</span>
                </div>
                <button class="aura-btn btn-fullscreen"><i class="fas fa-expand"></i></button>
            </div>
        </div>
    `;
    ...
    const bigPlay = wrapper.querySelector('.aura-big-play');
    const fullBtn = wrapper.querySelector('.btn-fullscreen');
    const progressBar = wrapper.querySelector('.aura-progress-bar');
    const progressContainer = wrapper.querySelector('.aura-progress-container');
    const timeDisplay = wrapper.querySelector('.aura-time');
