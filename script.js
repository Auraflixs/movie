...
// Esta función ahora se puede llamar directamente si el DOM ya está listo
...
// ARRANQUE SEGURO: Verifica si el DOM ya cargó antes de añadir el listener
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', iniciarTodo);
} else {
    // Si ya cargó (que pasa a menudo con cargadores dinámicos), iniciamos directo
    iniciarTodo();
}
...
function setupLatelyNew() {
    ...
    if (!container) return;
    let targetIds = [];
    if (window.LATELY_IDS && Array.isArray(window.LATELY_IDS)) targetIds = window.LATELY_IDS;
...
function openModal(id, typeHint) {
    ...
    if (item.genres && Array.isArray(item.genres)) {
        ...
        if (item.genres.length > 4) genreText += "...";
    } else if (item.genre) {
