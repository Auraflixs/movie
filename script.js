// script.js - Lógica arreglada para Matriz 3x10 y Búsqueda con Historial
...
// --- MANEJO DE HISTORIAL (BOTÓN ATRÁS) ---
window.onpopstate = function(event) {
    const state = event.state || {}; // Evitar error si state es null
    const modal = document.getElementById('videoModal');
    const search = document.getElementById('searchOverlay');

    // 1. Manejo del Modal
    if (modal.style.display === 'flex') {
        // Si el estado dice que NO hay modal, lo cerramos
        if (!state.modal) {
            closeModalInternal();
        }
    } else if (state.modal) {
        // Si el estado dice que SI hay modal (avance hacia adelante), lo restauramos (opcional, complejo de implementar perfecto sin datos)
    }
...
function closeModalInternal() {
    ...
    // Al cerrar modal, revisamos si debemos quitar bloqueo de scroll
    // Solo quitamos si el buscador NO está visible
    const search = document.getElementById('searchOverlay');
...
function switchView(viewName, pushToHistory = true) {
    ...
    // Nota: No cerramos el buscador aquí forzosamente, depende del contexto
    // Pero si cambiamos de vista principal, los botones de abajo cambian
    ...
    if (pushToHistory) {
        // Al cambiar de vista principal, asumimos que no hay modal ni búsqueda
        document.getElementById('searchOverlay').style.display = 'none';
        window.history.pushState({ view: viewName, modal: false, search: false }, '');
    }
}

// CAMBIO: Lógica Matriz 3 filas x 10 columnas
function renderHomeView() {
    ...
    renderMatrixList('homeMoviesRow', moviesShuffled, 'movies');
    renderMatrixList('homeSeriesRow', seriesShuffled, 'series');
}

function renderMatrixList(containerId, fullList, type) {
    const container = document.getElementById(containerId);
    if (!container) return;

    // LIMITE: 30 items (3 filas x 10 columnas)
    const LIMIT = 30;
    const itemsToShow = fullList.slice(0, LIMIT);
    
    // Creamos la estructura HTML interna para la matriz
    // Usamos un div interno 'matrix-grid' para forzar las columnas
    let gridHtml = itemsToShow.map(item => createItemHTML(item)).join('');

    // Botón Más
    if (fullList.length > LIMIT) {
        gridHtml += `
            <div class="item" onclick="switchView('${type}')">
                <div class="more-card">
                    Más >
                </div>
                <div class="item-title">Ver todas</div>
            </div>
        `;
    }

    container.innerHTML = `<div class="matrix-grid">${gridHtml}</div>`;
}
...
function openModal(id, type) {
    ...
    const isSearchOpen = searchOverlay.style.display === 'block';

    if (isSearchOpen) {
        // Ocultamos visualmente el buscador, pero NO lo cerramos lógicamente
        searchOverlay.style.display = 'none';
    }

    // PUSH STATE: Guardamos si el buscador estaba abierto
    window.history.pushState({ view: currentView, modal: true, search: isSearchOpen }, '');
...
function setupEventListeners() {
    ...
    document.getElementById('closeModal').onclick = () => { 
        window.history.back(); // Usa el historial para cerrar
    };
    ...
    document.getElementById('nav-search').onclick = (e) => {
        e.preventDefault();
        // Guardamos estado: vista actual, sin modal, con búsqueda true
