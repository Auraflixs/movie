...
function openModal(id, type) {
    ...
    // CORRECCIÓN: Buscamos en todo el contenido para asegurar que se encuentre
    // sin importar si se pasó 'movies' o 'series' incorrectamente.
    const allContent = [...moviesListInternal, ...seriesListInternal];
    const item = allContent.find(i => String(i.id) === idStr);
    
    if (!item) {
        console.error("No se encontró el contenido con ID:", id);
        return;
    }
    ...
    // LÓGICA DE VISUALIZACIÓN DE GÉNERO
    if (item.genre && item.genre.trim() !== "") {
        genreEl.innerText = item.genre;
        sepEl.classList.remove('hidden');
        genreEl.classList.remove('hidden');
    } else {
        // Si no hay género, ocultamos el separador y el texto
    ...
    // Verificar si es serie (tiene seasons) o película
...
function renderRealRecommendations(currentId) {
    ...
    // Mezclar todo y filtrar el actual
    ...
    container.innerHTML = selection.map(item => {
        // Asegurar que el tipo se pase correctamente en el onclick
        const type = item.seasons ? 'series' : 'movies';
        return `
            <div class="item" onclick="openModal('${item.id}', '${type}')">
                <img src="${item.image}">
