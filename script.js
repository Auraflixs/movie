...
function initApp() {
    renderHomeView();
    // Invertir lista para "Recién agregadas"
...
function renderHomeView() {
    ...
    // Aleatorio para mostrar variedad
    shuffleArray(moviesShuffled);
    shuffleArray(seriesShuffled);
    
    // CORRECCIÓN APLICADA: Ahora se mandan listas a renderMultiRow
...
function renderMultiRow(containerId, list) {
    ...
    // Antes estaba 'let i = 3; i < 3', lo cual no ejecutaba nunca.
    // Ahora es 'let i = 0', creará 3 filas (0, 1, 2)
    for (let i = 0; i < 3; i++) {
        const start = i * 10;
        const end = start + 10;
...
function openModal(id, type) {
    ...
    // Obtenemos los dos spans del HTML
    ...
    // --- CORRECCIÓN ORDEN Y COLORES ---
    // Usamos 'modalYear' para la parte MORADA: AÑO + PELÍCULA/SERIE
    const typeLabel = type === 'movies' ? 'Película' : 'Serie';
    yearEl.innerText = `${item.year} ${typeLabel}`;
    
    // Usamos 'modalType' para la parte GRIS: GÉNEROS
    if (item.genres && item.genres.length > 0) {
        let displayGenres = item.genres.slice(0, 4).join('/');
        if (item.genres.length > 4) {
            displayGenres += '...';
        }
        // Agregamos el punto aquí
        typeEl.innerText = ` • ${displayGenres}`;
    } else {
        typeEl.innerText = '';
    }
    // ------------------------------------
