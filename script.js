...
function loadUserDataInUI() {
    ...
    // Check VIP UI al cargar usuario
...
function openModal(id, typeHint) {
    ...
    // --- LÓGICA VIP PARA SERIES Y PELÍCULAS ---
    if (isSeries) {
        ...
        if (item.seasons && item.seasons.length > 0) {
            ...
            // Render episodios NO reproduce automáticamente ahora, solo lista
            renderEpisodes(item.seasons[0], item, -1); 
        ...
        // Limpiamos player al abrir modal de serie
        ...
        // Al abrir, cargamos el player con la película, PERO verificamos VIP
        if (isUserVip()) {
            setPlayerVideo(item.video); 
            addToContinueWatching(item, 'movies');
        } else {
            // Si no es VIP, mostramos el bloqueo
            showVipBlocker();
        }
...
function openSeasonSelectorModal() {
    ...
    currentModalItem.seasons.forEach((seasonObj, index) => {
        ...
        itemBtn.onclick = () => {
            ...
            renderEpisodes(seasonObj, currentModalItem, -1); 
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    // Autoplay logic (Solo si se pasara índice y fuera VIP, pero hemos puesto -1 por defecto arriba)
    ...
    playerDiv.innerHTML = `
        <div class="video-container" style="position:relative;">
            <div class="vip-player-overlay">
                ...
                <p class="vip-overlay-text">No eres miembro VIP</p>
...
// Función global para el botón del blocker
window.redirectToVipProfile = function() {
    closeModalInternal();
    switchView('profile');
    // Opcional: Abrir modal VIP directamente
