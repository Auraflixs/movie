...
function showVipBlocker() {
    ...
    // Generamos el HTML del bloqueo con la clase semitransparente y la imagen de fondo
...
// =============================================================
// FUNCIÓN PLAYER VIDEO ACTUALIZADA (SOPORTE LANDSCAPE)
// =============================================================
function setPlayerVideo(url, overlayText = null) {
    ...
    container.id = 'activeVideoContainer'; // ID para referencia

    // --- BOTÓN DE PANTALLA COMPLETA / LANDSCAPE ---
    // Este botón permite forzar la vista horizontal y maximizar
    ...
    // Lógica para alternar el modo Landscape forzado
    fsBtn.onclick = function() {
        const pWrapper = document.getElementById('modalContentPlayer');
        if (pWrapper.classList.contains('video-force-landscape')) {
            // Salir de modo landscape
            pWrapper.classList.remove('video-force-landscape');
            fsBtn.innerHTML = '<i class="fas fa-expand"></i>';
            if (document.exitFullscreen) document.exitFullscreen().catch(e=>{});
        } else {
            // Entrar en modo landscape
            pWrapper.classList.add('video-force-landscape');
            fsBtn.innerHTML = '<i class="fas fa-compress"></i>';
            if (pWrapper.requestFullscreen) pWrapper.requestFullscreen().catch(e=>{});
        }
    };
...
function closeModalInternal() {
    const modal = document.getElementById('videoModal');
    // Importante: Eliminar clase landscape si estaba activa
    const pWrapper = document.getElementById('modalContentPlayer');
    if(pWrapper) pWrapper.classList.remove('video-force-landscape');
...
function setupDiagnosis() {
    ...
    if (btn) btn.onclick = startLinkDiagnosis;
...
async function startLinkDiagnosis() {
    if (!confirm("Este proceso revisará todos los enlaces disponibles. Puede tardar varios minutos y consumir datos. ¿Continuar?")) return;
    ...
    // Recopilar todos los items a revisar
    let tasks = [];

    // 1. Películas
    window.moviesList.forEach(m => {
        tasks.push({ type: 'movie', title: m.title, url: m.video, id: m.id });
    });

    // 2. Series (Revisamos cada capítulo)
            ...
            s.seasons.forEach(season => {
                if (season.episodes) {
                    season.episodes.forEach(ep => {
                        tasks.push({ 
                            type: 'episode', 
                            title: `${s.title} (Cap ${ep.episode})`, // Solo Título y Capítulo
                            url: ep.video, 
                            id: s.id 
                        });
                    });
