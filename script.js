...
window.addEventListener('popstate', (event) => {
    ...
    // Si el usuario regresa y no hay modal en el estado, cerramos visualmente
    ...
    // Si el usuario regresa y no hay búsqueda en el estado, cerramos búsqueda
...
function initApp() {
    ...
    // Recién agregadas (Mantiene el orden de carga)
    ...
    setupHero(); // Aquí cargamos el Hero personalizado
    setupEventListeners();
    switchView('home', false);
}

// --- HERO: LÓGICA DE BÚSQUEDA POR ID ---
function setupHero() {
    const allContent = [...moviesListInternal, ...seriesListInternal];
    featuredList = [];

    // Verificamos si definiste IDs en id.js
    if (window.HERO_IDS && Array.isArray(window.HERO_IDS) && window.HERO_IDS.length > 0) {
        
        // Recorremos tu lista de IDs (ej: ["12", "13", "s2"])
        window.HERO_IDS.forEach(targetId => {
            // Buscamos en todas las películas/series cual tiene ese ID
            // Usamos String() para asegurar que "12" sea igual a 12
            const foundItem = allContent.find(item => String(item.id) === String(targetId));
            
            if (foundItem) {
                featuredList.push(foundItem);
            }
        });
    }

    // FALLBACK: Si no encontraste ninguna ID o la lista estaba vacía, poner las 5 primeras
    if (featuredList.length === 0) {
        console.warn("No se encontraron IDs del Hero, usando automáticas.");
...
// --- RESTO DE FUNCIONES (Vistas, Listas, etc) ---
...
function setupEventListeners() {
    ...
    // BOTÓN DE BUSCAR ARREGLADO
    document.getElementById('nav-search').onclick = (e) => {
        e.preventDefault();
        document.getElementById('searchOverlay').style.display = 'block';
        document.getElementById('searchInput').focus(); // Foco automático en el input
        window.history.pushState({ view: currentView, modal: false, search: true }, '');
    };

    // Botón Cerrar Modal (Flecha atrás interna)
    document.getElementById('closeModal').addEventListener('click', () => {
        window.history.back(); // Simula el botón del celular
    });

    // Botón Cerrar Buscador (X)
