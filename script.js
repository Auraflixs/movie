...
function openSeasonSelectorModal() {
    ...
    currentModalItem.seasons.forEach((seasonObj, index) => {
        ...
        // Estructura HTML del botón (Texto + Círculo Radio)
        itemBtn.innerHTML = `
        ...
        itemBtn.onclick = () => {
            ...
            // Actualizar texto del botón principal en la vista de la serie
            document.getElementById('currentSeasonText').innerText = `Temporada ${seasonObj.season}`;
            
            // Renderizar episodios de la nueva temporada seleccionada
            renderEpisodes(seasonObj, currentModalItem, 0); // 0 = reproducir primer cap
...
function setupEventListeners() {
    ...
    // NUEVO: Cerrar modal de temporadas
...
window.addEventListener('popstate', (event) => {
    ...
    if (!event.state || !event.state.modal) {
        ...
        document.getElementById('changeAvatarModal').style.display = 'none';
        document.getElementById('shareModal').style.display = 'none';
        document.getElementById('seasonSelectorModal').style.display = 'none'; // Cerrar modal de temporadas
    }
