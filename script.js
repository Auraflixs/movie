...
async function checkLink(item, logContainer) {
    ...
    try {
        ...
        // IMPORTANTE: 'no-cors' permite conectar a otros servidores pero
        // no deja leer el status (da status 0).
        // Si hay error de red (DNS, timeout), salta al catch.
        await fetch(url, { 
            method: 'GET', 
            mode: 'no-cors', 
        ...
        // Si llegamos aquí, hubo conexión exitosa (Info en morado)
        logToConsole(logContainer, item.title, "Online", "info");

    } catch (error) {
        if (error.name === 'AbortError') {
            logToConsole(logContainer, item.title, "Timeout (Lento)", "warning");
        } else {
            // Error real de red (dominio no existe o rechazado)
