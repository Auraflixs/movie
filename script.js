...
function renderHomeView() {
    ...
    // AQUÍ ESTABA EL PROBLEMA DE LAS DESTACADAS: Llamamos correctamente a renderMultiRow
...
function renderMultiRow(containerId, list) {
    ...
    // CORRECCIÓN: El bucle empieza en 0 y termina en 3 (antes era i=3)
    for (let i = 0; i < 3; i++) {
        const start = i * 10;
        const end = start + 10;
...
function openModal(id, type) {
    ...
    // SELECCIONAMOS EL CONTENEDOR PADRE DIRECTAMENTE
    ...
    // --- LÓGICA CORREGIDA AÑO Y GÉNEROS ---
    ...
    if (item.genres && item.genres.length > 0) {
        ...
        genresText = ` • ${displayGenres}`;
    }

    // INYECTAMOS EL HTML DIRECTAMENTE PARA FORZAR ORDEN Y COLOR
    // meta-main-text = MORADO (Año + Película/Serie)
    // meta-sub-text = GRIS ( • Generos)
    metaContainer.innerHTML = `
        <span class="meta-main-text">${item.year} ${typeLabel}</span>
        <span class="meta-sub-text">${genresText}</span>
    `;
    // --------------------------------------
