...
function openModal(id, type) {
    ...
    // Referencias a los spans del año/tipo
    ...
    // --- LÓGICA MODIFICADA PARA AÑO Y GÉNEROS ---
    
    // 1. Determinar etiqueta (Película o Serie)
    const typeLabel = type === 'movies' ? 'Película' : 'Serie';
    
    // 2. Mostrar "2025 Película" en morado (usando el elemento modalYear)
    yearEl.innerText = `${item.year} ${typeLabel}`;
    
    // 3. Mostrar "• Terror/aventura..." en gris (usando el elemento modalType)
    if (item.genres && item.genres.length > 0) {
        // Tomar máximo 4 géneros
        let displayGenres = item.genres.slice(0, 4).join('/');
        // Si hay más de 4, agregar puntos suspensivos
